const AUTO_PROVIDER_ORDER = ['ollama', 'nvidia', 'groq', 'openrouter'];
...
async function callNvidia({ system, prompt, model: modelOverride }) {
  const apiKey = providerApiKey('nvidia');
  if (!apiKey) {
    throw new Error('NVIDIA_API_KEY is not configured.');
  }

  const model = configuredModel('nvidia', { model: modelOverride });
...
async function callGroq({ system, prompt, model: modelOverride }) {
  const apiKey = providerApiKey('groq');
  if (!apiKey) {
    throw new Error('GROQ_API_KEY is not configured.');
  }

  const model = configuredModel('groq', { model: modelOverride });
...
async function callOpenRouter({ system, prompt, model: modelOverride }) {
  const apiKey = providerApiKey('openrouter');
  const model = configuredModel('openrouter', { model: modelOverride });
  if (!apiKey || !model) {
...
export async function runLLM({ system = '', prompt = '', provider = 'auto', model = '', baseUrl = '' }) {
  ...
  if (providerOrder.length === 0) {
    throw new Error(
      'No providers are configured. Set AIRA_OLLAMA_MODEL or OLLAMA_MODEL for Ollama, or configure NVIDIA/Groq/OpenRouter env vars.'
    );
  }
