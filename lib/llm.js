...
async function callGemini({ system, prompt, model: modelOverride }) {
  const apiKey = providerApiKey('gemini');
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY or GOOGLE_API_KEY is not configured.');
  }

  const model = configuredModel('gemini', { model: modelOverride });
...
export async function runLLM({ system = '', prompt = '', provider = 'auto', model = '', baseUrl = '' }) {
  ...
  const providerOrder = provider === 'auto'
    ? (await configuredProviders({ model, baseUrl })).providers
    : [provider];
